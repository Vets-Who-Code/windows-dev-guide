...
the `--production` flag will only install the modules from the `dependencies` section of `package.json` and ignore the `devDependenices`. The perk of this is notably reducing the size of the `node_modules` folder.
...
Check out the official [npm](https://docs.npmjs.com/) and [Node.js](https://nodejs.org/en/docs/) docs for more in-depth guides.
